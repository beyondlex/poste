  ...
  for idx, entry in ipairs(responses) do
    ...
    for _, view in ipairs(views) do
      ...
      if view == "body" and ft == "json" then
        local json = require("poste_http.http.json")
...
setup_keymaps = function(buf)
  ...
  if k then
    vim.keymap.set("n", k, function()
      ...
      require("poste_http").run_request()
    end, opts)
  end
  ...
  if k then
    vim.keymap.set("n", k, function()
      ...
      require("poste_http.http.json").start_interactive_input()
    end, opts)
  end
  ...
  if k then
    vim.keymap.set("n", k, function()
      local bufnr = vim.api.nvim_get_current_buf()
      if vim.bo[bufnr].filetype ~= "json" then return end
      require("poste_http.http.json").restore_original()
    end, opts)
  end
